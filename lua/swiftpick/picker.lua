...
---@field open_picker fun(opts?: SwiftpickOpenPickerOpts)

---@class SwiftpickOpenPickerOpts
...
local M = {
  ...
  ---@param opts? SwiftpickOpenPickerOpts options to customize the picker behavior
  open_picker = function(opts)
    if state.opened then
      return
    end
    -- Resolve the nil/bool tri-state: nil → config default, bool → explicit override.
    if opts and opts.global_picker ~= nil then
      state.global_picker = opts.global_picker
    else
    ...
    window.create_picker_window(on_close_picker)
  end,
}
